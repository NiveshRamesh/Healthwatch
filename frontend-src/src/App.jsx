  ...
  return (
    <div style={{ position:'relative', zIndex:1, maxWidth:860, margin:'0 auto', padding:'32px 20px 60px' }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:36, flexWrap:'wrap', gap:16 }}>
        <div>
          <h1 style={{ fontFamily:'var(--mono)', fontSize:'1.6rem', letterSpacing:'-0.5px', background:'linear-gradient(135deg,var(--accent),var(--accent2))', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent' }}>
            ⬡ HealthWatch
          </h1>
          <p style={{ fontSize:'0.82rem', color:'var(--muted)', marginTop:4, fontFamily:'var(--mono)' }}>
            Infrastructure Diagnostics · Self-Monitor Service
          </p>
        </div>
        <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:10 }}>
          <button
            ...
            style={{
              ...
              border:'none', borderRadius:8, padding:'10px 20px', cursor: running ? 'not-allowed' : 'pointer',
              opacity: running ? 0.5 : 1, letterSpacing:'0.5px',
            }}
          >
            {running
              ? <><Spinner /> RUNNING...</>
          ...
          <div style={{ fontFamily:'var(--mono)', fontSize:'0.72rem', color:'var(--muted)', background:'var(--surface)', border:'1px solid var(--border)', padding:'6px 12px', borderRadius:6 }}>
      ...
      {/* ── Overall bar ── */}
      <div style={{ display:'flex', alignItems:'center', gap:14, background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:'16px 22px', marginBottom:20 }}>
      ...
      {hasData && (
        <div style={{ display:'flex', gap:10, marginBottom:28, flexWrap:'wrap' }}>
          {[['ok', okC, 'Healthy', '16,185,129'], ['warn', warnC, 'Warning', '245,158,11'], ['error', errC, 'Failed', '239,68,68']].map(([type, count, label, rgb]) => (
            <div key={type} style={{ display:'flex', alignItems:'center', gap:6, background:'var(--surface)', border:`1px solid rgba(${rgb},0.25)`, borderRadius:20, padding:'6px 14px', fontSize:'0.78rem', fontFamily:'var(--mono)', color:`var(--${type==='ok'?'ok':type==='warn'?'warn':'error'})` }}>
              <div style={{ width:8, height:8, borderRadius:'50%', background:`var(--${type==='ok'?'ok':type==='warn'?'warn':'error'})`, boxShadow:`0 0 6px var(--${type==='ok'?'ok':type==='warn'?'warn':'error'})` }} />
              {count} {label}
            </div>
          ))}
        </div>
      ...
      {running && !hasData
        ...
        : !hasData
          ...
          : Object.entries(results).map(([key, checks]) => {
      ...
      <div style={{ textAlign:'center', marginTop:48, fontFamily:'var(--mono)', fontSize:'0.72rem', color:'var(--muted)' }}>
        ...
        Service: <span style={{ color:'var(--accent)' }}>healthwatch-pod</span>
      </div>
    </div>
  );
}
...
function Spinner() {
  return <span style={{ width:14, height:14, border:'2px solid rgba(0,0,0,0.3)', borderTopColor:'#0a0e1a', borderRadius:'50%', animation:'spin 0.7s linear infinite', display:'inline-block' }} />;
}
  ...
  return (
    <>
      {Object.entries(SECTIONS_META).map(([key, meta]) => (
        <div key={key} style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:14, marginBottom:16, overflow:'hidden' }}>
          ...
          {[1,2,3].map(i => (
            <div key={i} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 22px 14px 70px' }}>
              <div style={{ height:14, width:160, background:'var(--surface2)', borderRadius:4, animation:'shimmer 1.5s infinite' }} />
              <div style={{ height:22, width:70, background:'var(--surface2)', borderRadius:6, animation:'shimmer 1.5s infinite' }} />
            </div>
          ))}
        </div>
