...
POSTGRES_DSN = os.getenv(
    "POSTGRES_DSN", "postgresql://user:password@postgres:5432/appdb"
)
...
def _fetch_pod_logs(v1, pod_name: str, namespace: str, tail: int = 30) -> str:
    ...
    try:
        logs = v1.read_namespaced_pod_log(
            name=pod_name, namespace=namespace, tail_lines=tail, timestamps=False
        )
        return logs.strip()
    except Exception:
        try:
            logs = v1.read_namespaced_pod_log(
                name=pod_name, namespace=namespace, tail_lines=tail, previous=True
            )
...
def _get_pod_health(pod) -> dict:
    ...
    all_cs = (pod.status.init_container_statuses or []) + (
    ...
    for c in all_cs:
        ...
        if last and last.terminated and last.terminated.reason == "OOMKilled":
            if result["status"] == "ok":
                ...
                result["message"] = (
                    f"Last restart was OOM. Total restarts: {result['restarts']}"
                )
    ...
    def process_pods(pods, ready_key):
        ...
        for pod in pods:
            ...
            if health["status"] in ("error", "warn"):
                health["logs"] = _fetch_pod_logs(
                    v1, pod.metadata.name, K8S_NAMESPACE, tail=30
                )
...
def _parse_topic_describe(raw: str) -> dict:
    ...
    for line in raw.splitlines():
        ...
        if line.startswith("Topic:") and "Partition:" not in line:
            m = re.match(
                r"Topic:\s+(\S+)\s+PartitionCount:\s+(\d+)\s+ReplicationFactor:\s+(\d+)\s*(?:Configs:\s*(.*))?",
                line,
            )
            if m:
            ...
            m = re.match(
                r"Topic:\s+\S+\s+Partition:\s+(\d+)\s+Leader:\s+(-?\d+)\s+Replicas:\s+([\d,]+)\s+Isr:\s+([\d,]*)",
                line,
            )
            if m:
...
def _parse_consumer_groups(raw: str) -> dict:
    ...
    for line in raw.splitlines():
        ...
        topic_lag[topic]["groups"][group_id].append(
...
async def check_kafka() -> dict:
    ...
    try:
        ...
        checks["Pod Health"] = {
            "status": "error",
            "detail": f"Could not check pods: {e}",
        }
        return checks
    ...
    for pod_name, ph in pod_health["pods"].items():
        icon = (
            "🟢"
            if ph["status"] == "ok"
            else ("🔴" if ph["status"] == "error" else "🟡")
        )
        detail_parts = [f"Phase: {ph['phase']}"]
        if ph["reason"]:
            detail_parts.append(ph["reason"])
        if ph["restarts"]:
            detail_parts.append(f"Restarts: {ph['restarts']}")
        if ph["message"]:
            detail_parts.append(ph["message"])
    ...
    if not pod_health["broker_ready"]:
        ...
        checks["Live Data"] = {
            "status": "unknown",
            "detail": "Skipped — broker not ready",
        }
        checks["Consumer Lag"] = {
            "status": "unknown",
            "detail": "Skipped — broker not ready",
    ...
    try:
        ...
        if "usable" in raw or "-> (" in raw:
            ...
            info = (
                f"Broker ID: {m.group(2)} | Host: {m.group(1)}"
                if m
                else "Broker responding"
            )
            checks["Broker"] = {"status": "ok", "detail": info}
        else:
            checks["Broker"] = {
                "status": "error",
                "detail": "No API response from broker",
            }
    except Exception as e:
    ...
    try:
        ...
        broker_id = next(
            (
                ...
                if l.startswith("cluster.id=")
            ),
            None,
        )

        zk_raw = await loop.run_in_executor(
            None,
            lambda: _k8s_exec(
                ZOOKEEPER_POD,
                K8S_NAMESPACE,
        ...
        if broker_id and zk_id:
            ...
            checks["Cluster ID"] = {
                "status": "warn",
                "detail": f"Could not verify (broker:{broker_id}, zk:{zk_id})",
            }
    except Exception as e:
    ...
    try:
        ...
        if offline_topics:
            checks["Topics"] = {
                "status": "error",
                "detail": f"{len(topic_data)} topics | {len(offline_topics)} have OFFLINE partitions: {', '.join(offline_topics[:3])}",
            }
        elif under_replicated:
            checks["Topics"] = {
                "status": "warn",
                "detail": f"{len(topic_data)} topics | {len(under_replicated)} under-replicated: {', '.join(under_replicated[:3])}",
            }
        else:
            checks["Topics"] = {
                "status": "ok",
                "detail": f"{len(topic_data)} topics — all partitions healthy",
            }
    ...
    try:
        ...
        raw_earliest = await loop.run_in_executor(
            None,
            lambda: _k8s_exec(
                KAFKA_POD,
                K8S_NAMESPACE,
        ...
        stale_topics = [
            t for t, s in topic_live.items() if s["has_data"] and not s["is_live"]
        ]
        empty_topics = [t for t, s in topic_live.items() if not s["has_data"]]

        parts = [f"{len(live_topics)} live"]
        if stale_topics:
            parts.append(f"{len(stale_topics)} stale (no new msgs)")
        if empty_topics:
            parts.append(f"{len(empty_topics)} empty")
    ...
    try:
        ...
        high_lag_topics = {
            t: d for t, d in lag_data.items() if d["total_lag"] > KAFKA_LAG_THRESHOLD
        }

        if high_lag_topics:
            top = sorted(
                high_lag_topics.items(), key=lambda x: x[1]["total_lag"], reverse=True
            )[:3]
            checks["Consumer Lag"] = {
                "status": "warn",
                "detail": "⚠️ HIGH LAG: "
                + ", ".join([f"{t} ({d['total_lag']:,})" for t, d in top]),
            }
        else:
    ...
    try:
        if topic_data:
            ...
            checks["ISR Sync"] = {
                ...
                "detail": "All partitions fully in-sync"
                if under == 0
                else f"{under} topics under-replicated (ISR < Replicas)",
            }
    except Exception as e:
    ...
    if pod_health["zookeeper_ready"]:
        try:
            zk_ruok = await loop.run_in_executor(
                None,
                lambda: _k8s_exec(
                    ZOOKEEPER_POD,
                    K8S_NAMESPACE,
            ...
            checks["Zookeeper Health"] = {
                ...
                "detail": "imok ✓"
                if "imok" in zk_ruok
                else f"Bad response: {zk_ruok.strip()}",
            }

            zk_stat = await loop.run_in_executor(
                None,
                lambda: _k8s_exec(
                    ZOOKEEPER_POD,
                    K8S_NAMESPACE,
            ...
            zk_mntr = await loop.run_in_executor(
                None,
                lambda: _k8s_exec(
                    ZOOKEEPER_POD,
                    K8S_NAMESPACE,
            ...
            if avg_lat > 100:
                zk_d += " ⚠️ High latency"
            if out_req > 10:
                zk_d += " ⚠️ High outstanding requests"
...
async def get_topic_detail(topic_name: str) -> dict:
    ...
    try:
        ...
        raw_lag = await loop.run_in_executor(
            None,
            lambda: _k8s_exec(
                KAFKA_POD,
                K8S_NAMESPACE,
...
async def check_postgres() -> dict:
    ...
    try:
        ...
        try:
            ...
            checks["Query Execution"] = {
                "status": "ok",
                "detail": "Queries running normally",
            }
        except Exception as e:
    ...
    try:
        async with httpx.AsyncClient(timeout=5) as http:
            ...
            checks["Liveness"] = {
                ...
                "detail": "MinIO is live"
                if r.status_code == 200
                else f"Status {r.status_code}",
            }
            checks["Readiness"] = {
                "status": "ok" if r2.status_code == 200 else "warn",
                "detail": "MinIO is ready"
                if r2.status_code == 200
                else f"Status {r2.status_code}",
            }
    except Exception as e:
...
async def check_kubernetes() -> dict:
    ...
    try:
        ...
        for pod_prefix in MONITORED_PODS:
            ...
            matching = [
                p for name, p in pod_map.items() if pod_prefix.lower() in name.lower()
            ]
            if not matching:
                checks[f"{pod_prefix.capitalize()} Pod"] = {
                    "status": "error",
                    "detail": "No pod found",
                }
                continue
            ...
            if phase == "Running":
                ...
                if all_ready and restarts < 5:
                    checks[f"{pod_prefix.capitalize()} Pod"] = {
                        "status": "ok",
                        "detail": f"{name} — Running, {restarts} restarts",
                    }
                elif restarts >= 5:
                    checks[f"{pod_prefix.capitalize()} Pod"] = {
                        "status": "warn",
                        "detail": f"{name} — {restarts} restarts (high)",
                    }
                else:
                    checks[f"{pod_prefix.capitalize()} Pod"] = {
                        "status": "warn",
                        "detail": f"{name} — Not all containers ready",
                    }
            elif phase == "Pending":
                checks[f"{pod_prefix.capitalize()} Pod"] = {
                    "status": "warn",
                    "detail": f"{name} — Pending",
                }
            else:
                checks[f"{pod_prefix.capitalize()} Pod"] = {
                    "status": "error",
...
async def run_all_checks():
    ...
    try:
        ...
        for label, res in zip(labels, results):
            last_result[label] = (
                {"Error": {"status": "error", "detail": str(res)}}
...
# ── CORS — allows React dev server (localhost:3000) to call the API ──
app.add_middleware(
    CORSMiddleware,
...
# ── Serve React build — must be LAST, catches all non-API routes ─────
...
async def serve_react(full_path: str):
    ...
    return {
        "message": "React build not found — run npm run build or use npm start on port 3000"
    }
