...
CLICKHOUSE_HOST           = os.getenv("CLICKHOUSE_HOST",          "chi-clickhouse-vusmart-0.chi-clickhouse-vusmart.vsmaps.svc.cluster.local")
CLICKHOUSE_PORT           = int(os.getenv("CLICKHOUSE_PORT",      "9000"))
CLICKHOUSE_USER           = os.getenv("CLICKHOUSE_USER",          "default")
CLICKHOUSE_PASSWORD       = os.getenv("CLICKHOUSE_PASSWORD",      "")
CLICKHOUSE_DB             = os.getenv("CLICKHOUSE_DB",            "vusmart")
LONGHORN_URL              = os.getenv("LONGHORN_URL",             "http://longhorn-backend.longhorn-system.svc.cluster.local:9500")
KAFKA_REQUIRED_CONNECTORS = os.getenv("KAFKA_REQUIRED_CONNECTORS","enrichment-connector")
...
MONITORED_PODS            = os.getenv("MONITORED_PODS",
...
MINIO_ENDPOINT            = os.getenv("MINIO_ENDPOINT",  "http://minio.vsmaps.svc.cluster.local:9000")
...
def _get_ch():
    ...
    return clickhouse_connect.get_client(
        ...
        database=CLICKHOUSE_DB, connect_timeout=10, query_limit=0,
    )
...
def _mem_to_bytes(v: str) -> int:
    for s, m in {"Ki":1024,"Mi":1024**2,"Gi":1024**3,"Ti":1024**4,
                  "K":1000,"M":1000**2,"G":1000**3,"T":1000**4}.items():
        if v.endswith(s):
...
async def check_kafka_connectors() -> dict:
    t0 = time.time()
    logger.info(f"[kafka_connectors] START ns={K8S_NAMESPACE} required={KAFKA_REQUIRED_CONNECTORS}")
    try:
        core, _, _ = _get_k8s()
        ...
        if not pods:
            ...
            return {"pod_status":"Missing","active":[],"required":[],"missing":[],
                    "status":"critical","detail":"No cp-kafka-connect pod found"}

        pod_name   = pods[0].metadata.name
        pod_status = pods[0].status.phase
        logger.info(f"[kafka_connectors] pod={pod_name} phase={pod_status}")

        cmd = ["kubectl","exec",pod_name,"-n",K8S_NAMESPACE,"--","curl","-s","localhost:9082/connectors"]
        logger.debug(f"[kafka_connectors] exec: {' '.join(cmd)}")
        res = subprocess.run(cmd, capture_output=True, text=True, timeout=15)
        if res.returncode != 0:
            logger.warning(f"[kafka_connectors] exec rc={res.returncode} stderr={res.stderr.strip()!r}")
        ...
        try:
            ...
            logger.error(f"[kafka_connectors] JSON parse error: {je} raw={res.stdout.strip()!r}")
            active = []

        required = [c.strip() for c in KAFKA_REQUIRED_CONNECTORS.split(",")]
        missing  = [c for c in required if c not in active]
        logger.info(f"[kafka_connectors] active={active} required={required} missing={missing}")
        status = "critical" if pod_status != "Running" else ("warn" if missing else "ok")
        detail = f"{len(active)} connectors active" + (f" | MISSING: {missing}" if missing else "")
        logger.info(f"[kafka_connectors] status={status}")
        _timed("kafka_connectors", t0)
        return {"pod_status":pod_status,"active":active,"required":required,
                "missing":missing,"status":status,"detail":detail}
    except Exception as e:
        logger.error(f"[kafka_connectors] EXCEPTION: {e}\n{traceback.format_exc()}")
        _timed("kafka_connectors", t0)
        return {"pod_status":"Error","active":[],"required":[],"missing":[],
...
async def check_longhorn() -> dict:
    t0 = time.time()
    logger.info(f"[longhorn] START url={LONGHORN_URL} "
    ...
    try:
        ...
        logger.info(f"[longhorn] /v1/volumes HTTP {vr.status_code}  /v1/nodes HTTP {nr.status_code}")
        ...
        _timed("longhorn", t0)
        return {"volumes":[],"nodes":[],"status":"error","detail":f"Longhorn unreachable: {e}"}

    vol_results = []
    for v in volumes_raw:
        ab = int(v.get("actualSize",0)); cb = int(v.get("size",1))
        pct   = ab/cb*100 if cb > 0 else 0
        state = v.get("state","unknown"); ready = v.get("ready",False)
        pod   = next((kb.get("podName") for kb in v.get("kubeStatus",{}).get("workloadsStatus",[])
                      if kb.get("podStatus")=="Running"), None)
        status = "critical" if (not ready or state!="attached") else ("warn" if cb>0 and ab/cb>LH_ACTUAL_THRESHOLD else "ok")
        logger.debug(f"[longhorn] vol={v['name']} state={state} ready={ready} "
                     f"{round(ab/1024**3,1)}GiB/{round(cb/1024**3,1)}GiB → {status}")
        if status != "ok": logger.warning(f"[longhorn] vol={v['name']} → {status}")
        vol_results.append({"name":v["name"],"pod":pod,"pvc":v.get("kubeStatus",{}).get("pvcName",""),
                             "state":state,"ready":ready,"actual_gb":round(ab/1024**3,1),
                             "csize_gb":round(cb/1024**3,1),"used_pct":round(pct,1),"status":status})

    node_results = []
    for n in nodes_raw:
        for did, disk in n.get("disks",{}).items():
            sc = int(disk.get("storageScheduled",0)); mx = int(disk.get("storageMaximum",1))
            av = int(disk.get("storageAvailable",1))
            pct = sc/mx*100 if mx>0 else 0
            status = "warn" if (sc/mx if mx>0 else 0) > LH_NODE_FREE_THRESHOLD else "ok"
            logger.debug(f"[longhorn] node={n['name']} disk={did} "
                         f"sched={round(sc/1024**3,1)}GiB max={round(mx/1024**3,1)}GiB → {status}")
            node_results.append({"node":n["name"],"disk":did,"path":disk.get("path",""),
                                  "scheduled_gb":round(sc/1024**3,1),"available_gb":round(av/1024**3,1),
                                  "used_pct":round(pct,1),"status":status})
    ...
    logger.info(f"[longhorn] overall={overall} vols={len(vol_results)} nodes={len(node_results)}")
    _timed("longhorn", t0)
...
async def check_pvc_and_pods() -> dict:
    t0 = time.time()
    logger.info(f"[pods_pvcs] START ns={K8S_NAMESPACE} restart_thr={POD_RESTART_THRESHOLD}")
    try:
        core, _, _ = _get_k8s()
        pod_list = core.list_namespaced_pod(namespace=K8S_NAMESPACE).items
        pvc_list = core.list_namespaced_persistent_volume_claim(namespace=K8S_NAMESPACE).items
    ...
    for pod in pod_list:
        ...
        for c in (pod.status.container_statuses or []):
            ...
            state    = "running" if c.state.running else ("waiting" if c.state.waiting else "terminated")
            restarts = c.restart_count or 0
            if restarts > POD_RESTART_THRESHOLD:
                alerts.append(f"⚠ {c.name}: {restarts} restarts")
                logger.warning(f"[pods_pvcs] pod={pod.metadata.name} c={c.name} restarts={restarts}")
            if state != "running":
                alerts.append(f"⚠ {c.name}: state={state}")
                logger.warning(f"[pods_pvcs] pod={pod.metadata.name} c={c.name} state={state}")
            if not cpu_lim or not mem_lim:
                alerts.append(f"⚠ {c.name}: missing resource limits")
                logger.warning(f"[pods_pvcs] pod={pod.metadata.name} c={c.name} missing limits")
            containers.append({"name":c.name,"state":state,"restarts":restarts,
                                "cpu_limit":cpu_lim,"mem_limit":mem_lim,"cpu_req":cpu_req,"mem_req":mem_req})
        phase = pod.status.phase or "Unknown"
        s = ("warn" if phase=="Pending" else "error") if phase not in ("Running","Succeeded") else \
            ("warn" if any("restarts" in a or "state=" in a for a in alerts) else \
             ("warn" if any("missing resource" in a for a in alerts) else "ok"))
        if phase not in ("Running","Succeeded"):
            logger.warning(f"[pods_pvcs] pod={pod.metadata.name} phase={phase} → {s}")
        pod_results.append({"name":pod.metadata.name,"phase":phase,"containers":containers,"alerts":alerts,"status":s})
    ...
    for pvc in pvc_list:
        ...
        s = "critical" if phase=="Lost" else "warn" if phase=="Pending" or orphan else "ok"
        if s != "ok": logger.warning(f"[pods_pvcs] PVC={name} phase={phase} orphan={orphan} → {s}")
        pvc_results.append({"name":name,"phase":phase,"capacity":cap,"orphaned":orphan,"status":s})
...
async def check_clickhouse_tables() -> dict:
    t0 = time.time()
    logger.info(f"[ch_tables] START {CLICKHOUSE_HOST}:{CLICKHOUSE_PORT} db={CLICKHOUSE_DB} "
    ...
    try:
        ...
        if unused_kafka: logger.warning(f"[ch_tables] unused kafka tables: {unused_kafka}")

        r12 = q("Q12_readonly","SELECT database, table FROM system.replicas WHERE is_readonly=1")
        readonly = [{"database":r[0],"table":r[1]} for r in r12]
        if readonly: logger.warning(f"[ch_tables] readonly tables: {readonly}")

        r13 = q("Q13_inactive_ddl",
        ...
        mutations = [{"database":r[0],"table":r[1],"mutation_id":r[2],
                      "command":r[3],"create_time":r[4],"parts_to_do":r[5]} for r in r14]
        if mutations: logger.warning(f"[ch_tables] long mutations: {[m['mutation_id'] for m in mutations]}")
        ...
        detached = [{"database":r[0],"table":r[1],"reason":r[2],"count":r[3]} for r in r16]
        if detached: logger.warning(f"[ch_tables] detached parts: {detached}")

        r17 = q("Q17_table_sizes",
        ...
        stuck_repl = [{"database":r[0],"table":r[1],"type":r[2],"num_postponed":r[3]} for r in r18]
        if stuck_repl: logger.warning(f"[ch_tables] stuck replication: {stuck_repl}")

        r19 = q("Q19_replica_inconsistency",
        ...
        inconsistent = [{"database":r[0],"table":r[1],"replicas":r[2]} for r in r19]
        if inconsistent: logger.warning(f"[ch_tables] inconsistent replicas: {inconsistent}")

        _timed("ch_tables", t0)
        return {
            "unused_kafka_tables":  {"count":len(unused_kafka),"tables":unused_kafka,
                "status":"warn" if unused_kafka else "ok",
                "detail":f"{len(unused_kafka)} Kafka tables never committed" if unused_kafka else "All Kafka tables active"},
            "readonly_tables":      {"tables":readonly,
                "status":"critical" if readonly else "ok",
                "detail":f"{len(readonly)} read-only tables" if readonly else "No read-only tables"},
            "inactive_queries":     {"count":len(inactive_ddl),"queries":inactive_ddl,
                "status":"warn" if inactive_ddl else "ok",
                "detail":f"{len(inactive_ddl)} inactive DDL queries" if inactive_ddl else "No inactive DDL queries"},
            "long_mutations":       {"mutations":mutations,
                "status":"warn" if mutations else "ok",
                "detail":f"{len(mutations)} mutations >{CH_MUTATION_AGE_MINUTES}min" if mutations else "No long mutations"},
            "tables_without_ttl":   {"tables":no_ttl,
                "status":"warn" if no_ttl else "ok",
                "detail":f"{len(no_ttl)} tables missing TTL" if no_ttl else "All tables have TTL"},
            "detached_parts":       {"parts":detached,
                "status":"critical" if detached else "ok",
                "detail":f"{len(detached)} tables with detached parts" if detached else "No detached parts"},
            "table_sizes":          {"tables":table_sizes,"status":"ok","detail":"Top 5 table sizes"},
            "replication_stuck_jobs":{"jobs":stuck_repl,
                "status":"warn" if stuck_repl else "ok",
                "detail":f"{len(stuck_repl)} stuck replication jobs" if stuck_repl else "No stuck jobs"},
            "replica_inconsistency":{"tables":inconsistent,
                "status":"critical" if inconsistent else "ok",
                "detail":f"{len(inconsistent)} replica mismatches" if inconsistent else "All replicas consistent",
                "expected_replicas":2},
        }
    except Exception as e:
...
async def check_kubernetes_resources() -> dict:
    t0 = time.time()
    logger.info(f"[k8s_resources] START ns={K8S_NAMESPACE} "
    ...
    try:
        ...
        logger.info(f"[k8s_resources] {len(node_metrics.get('items',[]))} node metrics")
        ...
        for nm in node_metrics.get("items",[]):
            ...
            raw = nm["usage"]["cpu"]
            cpu_used = int(raw[:-1])/1e9 if raw.endswith("n") else _milli_to_cores(raw)
            ...
            s  = "critical" if cp>=90 or mp>=90 else "warn" if cp>=NODE_CPU_WARN_THRESHOLD or mp>=NODE_MEM_WARN_THRESHOLD else "ok"
            logger.info(f"[k8s_resources] node={name} cpu={cp:.1f}% mem={mp:.1f}% "
                        f"({round(mem_used/1024**3,1)}/{round(mem_total/1024**3,1)} GiB) → {s}")
            node_resources.append({"node":name,
                "cpu_used_pct":round(cp,1),"cpu_threshold":NODE_CPU_WARN_THRESHOLD,
                "memory_used_pct":round(mp,1),"memory_threshold":NODE_MEM_WARN_THRESHOLD,
                "memory_used_gb":round(mem_used/1024**3,1),"memory_total_gb":round(mem_total/1024**3,1),
                "cpu_used_cores":round(cpu_used,2),"cpu_total_cores":round(cpu_total,2),"status":s})

        pod_metrics = custom.list_namespaced_custom_object("metrics.k8s.io","v1beta1",K8S_NAMESPACE,"pods")
        pod_specs   = {p.metadata.name:p for p in core.list_namespaced_pod(namespace=K8S_NAMESPACE).items}
        ...
        for pm in pod_metrics.get("items",[]):
            ...
            cpu_used = sum(
                int(c["usage"]["cpu"][:-1])/1e9 if c["usage"]["cpu"].endswith("n")
                else _milli_to_cores(c["usage"]["cpu"])
                for c in pm.get("containers",[]))
            mem_used = sum(_mem_to_bytes(c["usage"]["memory"]) for c in pm.get("containers",[]))
            cpu_lim = mem_lim = 0
            ...
            s  = "critical" if cp>=90 or mp>=90 else "warn" if cp>=POD_CPU_WARN_THRESHOLD or mp>=POD_MEM_WARN_THRESHOLD else "ok"
            if s!="ok": logger.warning(f"[k8s_resources] pod={pname} cpu={cp:.1f}% mem={mp:.1f}% → {s}")
            else:       logger.debug(f"[k8s_resources] pod={pname} cpu={cp:.1f}% mem={mp:.1f}% → ok")
            pod_resources.append({"pod":pname,"namespace":K8S_NAMESPACE,
                "cpu_used_pct":round(cp,1),"memory_used_pct":round(mp,1),"status":s})

        all_s   = [n["status"] for n in node_resources]+[p["status"] for p in pod_resources]
        overall = "critical" if "critical" in all_s else "warn" if "warn" in all_s else "ok"
        logger.info(f"[k8s_resources] overall={overall}")
        _timed("k8s_resources", t0)
        return {"node_resources":node_resources,"pod_resources":pod_resources,"status":overall}
    except Exception as e:
        logger.error(f"[k8s_resources] EXCEPTION: {e}\n{traceback.format_exc()}")
        _timed("k8s_resources", t0)
        return {"node_resources":[],"pod_resources":[],"status":"error","detail":str(e)}
...
async def check_clickhouse_connectivity() -> dict:
    ...
    try:
        ...
        return {
            "Connection":      {"status":"ok","detail":f"Reachable on {CLICKHOUSE_HOST}:{CLICKHOUSE_PORT}"},
            "System Tables":   {"status":"ok","detail":f"{n} tables"},
            "Query Execution": {"status":"ok","detail":"Query executed successfully"},
        }
    except Exception as e:
        logger.error(f"[ch_connectivity] EXCEPTION: {e}\n{traceback.format_exc()}")
        _timed("ch_connectivity", t0)
        return {"Connection":{"status":"error","detail":str(e)},
...
async def check_kafka() -> dict:
    ...
    try:
        core,_,_ = _get_k8s()
        bp = core.list_namespaced_pod(namespace=K8S_NAMESPACE, label_selector="app=cp-kafka").items
        ...
        zp = core.list_namespaced_pod(namespace=K8S_NAMESPACE, label_selector="app=cp-zookeeper").items
        ...
        return {
            ...
            "Zookeeper Mode":   {"status":zs,"detail":f"{len(zr)}/{len(zp)} Zookeeper Running"},
            "Zookeeper Stats":  {"status":"ok","detail":"Stats exec not yet implemented"},
            "Live Data":        {"status":"ok","detail":"Topic live check via CH"},
            "Consumer Lag":     {"status":"ok","detail":"Lag check via CH kafka_consumers"},
            "Kafka Connectors": {"status":connectors["status"],"detail":connectors["detail"],"_connectors":connectors},
            "__details__":      {"topics":{},"topic_live_status":{},"consumer_lag":{}},
        }
    except Exception as e:
        logger.error(f"[kafka] EXCEPTION: {e}\n{traceback.format_exc()}")
        _timed("kafka", t0)
        return {"Broker Health":{"status":"error","detail":str(e)},
                "Zookeeper Mode":{"status":"error","detail":"N/A"},
                "Kafka Connectors":{"status":"error","detail":"N/A"},"__details__":{}}
...
async def check_postgres() -> dict:
    ...
    try:
        import asyncpg
        conn = await asyncpg.connect(host=POSTGRES_HOST,port=POSTGRES_PORT,
            user=POSTGRES_USER,password=POSTGRES_PASSWORD,database=POSTGRES_DB,timeout=10)
        ...
        _timed("postgres", t0)
        return {"Connection":{"status":"ok","detail":"Connected successfully"},
...
async def check_minio() -> dict:
    ...
    try:
        ...
        _timed("minio", t0)
        return {"Liveness":{"status":"ok" if lr.status_code==200 else "error","detail":f"HTTP {lr.status_code}"},
                "Readiness":{"status":"ok" if rr.status_code==200 else "error","detail":f"HTTP {rr.status_code}"}}
    except Exception as e:
        logger.error(f"[minio] EXCEPTION: {e}\n{traceback.format_exc()}")
        _timed("minio", t0)
        return {"Liveness":{"status":"error","detail":str(e)},"Readiness":{"status":"error","detail":str(e)}}
    ...
    try:
        ...
        for prefix in MONITORED_PODS:
            ...
            if not matched:
                ...
                checks[f"Pod: {prefix.capitalize()}"] = {"status":"warn","detail":f"No pod matching '{prefix}'"}
                continue
            running  = [p for p in matched if p.status.phase=="Running"]
            restarts = sum((cs.restart_count or 0) for p in matched for cs in (p.status.container_statuses or []))
            s = ("ok" if running else "error")
            if restarts > POD_RESTART_THRESHOLD and s == "ok": s = "warn"
            detail = f"{matched[0].metadata.name} — {matched[0].status.phase}, {restarts} restarts"
            logger.info(f"[k8s_pods] prefix={prefix!r} running={len(running)}/{len(matched)} restarts={restarts} → {s}")
            checks[f"Pod: {prefix.capitalize()}"] = {"status":s,"detail":detail}
        rn = sum(1 for n in all_nodes if any(c.type=="Ready" and c.status=="True" for c in n.status.conditions))
        ns = "ok" if rn==len(all_nodes) else "warn"
        logger.info(f"[k8s_pods] nodes ready={rn}/{len(all_nodes)} → {ns}")
        checks["Cluster Nodes"] = {"status":ns,"detail":f"{rn}/{len(all_nodes)} nodes Ready"}
        _timed("k8s_pods", t0)
        return checks
    except Exception as e:
...
async def run_all_checks():
    ...
    try:
        results = await asyncio.gather(
            check_clickhouse_connectivity(), check_kafka(), check_postgres(),
            check_minio(), check_kubernetes_pods(), check_longhorn(),
            check_pvc_and_pods(), check_clickhouse_tables(), check_kubernetes_resources(),
            return_exceptions=True,
        )
        names = ["ch_conn","kafka","pg","minio","k8s_pods","longhorn","pvc_pods","ch_tables","k8s_resources"]
        for name, r in zip(names, results):
            if isinstance(r, Exception):
                logger.error(f"[aggregate] top-level exception in {name}: {r}\n{traceback.format_exc()}")

        def safe(r, key):
            return {key:{"status":"error","detail":str(r)}} if isinstance(r, Exception) else r

        ch_conn, kafka, pg, minio, k8s_pods, longhorn, pvc_pods, ch_tables, k8s_resources = results
        ...
        last_result = {
            ...
            "kubernetes": {**safe(k8s_pods,"K8s Pods"), "__resources__": safe(k8s_resources,"K8s Resources")},
...
async def log_requests(request: Request, call_next):
    ...
    logger.debug(f"HTTP {request.method} {request.url.path} → {resp.status_code} ({round(time.time()-t0,3)}s)")
    return resp
...
async def startup():
    ...
    logger.info(f"  NODE thresholds cpu>{NODE_CPU_WARN_THRESHOLD}% mem>{NODE_MEM_WARN_THRESHOLD}%")
    logger.info(f"  POD  thresholds cpu>{POD_CPU_WARN_THRESHOLD}% mem>{POD_MEM_WARN_THRESHOLD}%")
...
async def get_status():
    return {"last_checked":last_checked,"is_running":is_running,
            "results":last_result,"timings":check_durations}
...
async def topic_detail(topic_name: str):
    ...
    try:
        ...
        parts = [{"partition":str(r[0]),"earliest":r[1],"latest":r[2],"messages":r[2]-r[1],"is_live":True} for r in rows]
        total = sum(p["messages"] for p in parts)
        logger.info(f"[topic_detail] {topic_name}: {len(parts)} partitions total={total}")
        return {"topic":topic_name,"found":True,"total_messages":total,"partition_offsets":parts}
    except Exception as e:
